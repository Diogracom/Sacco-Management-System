...
$currency = $row1['currency'];
  ...
  $sel = mysqli_query($link, "SELECT sum(amount) FROM transaction WHERE date_time between '$date1' and '$date2' && t_type = 'Deposit' ") or die (mysqli_error($link));
  $rows = mysqli_fetch_array($sel);


}
        echo "<br>";
		    echo "<font color='#555FFF'><br><b>Deposits Worthy &nbsp : &nbsp</b></font>" ;
...
echo "<div class='alert alert-info'>No data found yet!.....Check back later!!</div>";
	...
	$bag = mysqli_query($link, "SELECT * FROM transaction WHERE date_time between '$date1' and '$date2' && t_type = 'Deposit' ") or die (mysqli_error($link));
...
$currency = $row1['currency'];
  ...
  $sel = mysqli_query($link, "SELECT sum(amount) FROM transaction WHERE id = '$borrower' && date_time between '$date1' and '$date2' && t_type = 'Deposit' ") or die (mysqli_error($link));
  $rows = mysqli_fetch_array($sel);
...
echo "<div class='alert alert-info'>No data found yet!.....Check back later!!</div>";
	...
	$bag = mysqli_query($link, "SELECT * FROM transaction WHERE  id = '$account' && date_time between '$date1' and '$date2' && t_type = 'Deposit' ") or die (mysqli_error($link));
